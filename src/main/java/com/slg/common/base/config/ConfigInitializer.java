 ...
 * <li>-Dqm.config.external.path=/etc/squirrel-legend/config (optional)</li>
 * <p>
 * <p>
 ...
 * should be placed in /etc/squirrel-legend/config.
...
public class ConfigInitializer {
    ...
    static final String CONFIG_EXTERNAL_PATH_KEY = "qm.config.external.path";
    ...
    public void init() {
        ...
        //LoggingConfig.updateLogLevel(getBackingConfigurationSource());
     ...
     */
    static void cascadeLoadConfigFromClassPath(String baseFileName) {
        try {
     ...
     */
    static String getConfigExternalPath(Configuration config) {
        String configOverridePath = config.getString(CONFIG_EXTERNAL_PATH_KEY, "/etc/slg/config");
        if (!configOverridePath.endsWith("/"))
            configOverridePath = configOverridePath + "/";
        return configOverridePath;
    }

    static ConcurrentCompositeConfiguration getBackingConfigurationSource() {
